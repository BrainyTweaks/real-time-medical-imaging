%% Project 259 - CIRS040GSE batched GPU reconstruction
...
fprintf("Project 259 - CIRS040GSE batched GPU reconstruction\n");
fprintf("--------------------------------------------------\n");
fprintf("GPU: %s\n", gpu.Name);
...
sampleTime = gpuArray( ...
    single((0:numSamples-1) / USHEADER.fs));

wait(gpuDevice);

fprintf("Starting batched GPU beamforming...\n");
...
fprintf("Batched GPU reconstruction time: %.4f seconds\n", ...
    reconstructionTime);
...
outputFile = fullfile( ...
    outputDir, ...
    "cirs040_gpu_batched_reconstruction.mat");

imageFile = fullfile( ...
    outputDir, ...
    "cirs040_gpu_batched_reconstruction.png");
...
ylabel("Depth (mm)");
title("Project 259 - CIRS040GSE Batched GPU Reconstruction");
...
fprintf("\nBatched GPU reconstruction finished.\n");
